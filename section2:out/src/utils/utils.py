...
def med_reshape(image, new_shape):
    ...
    x_offset = (new_shape[0] - image.shape[0]) // 2
    y_offset = (new_shape[1] - image.shape[1]) // 2
    z_offset = (new_shape[2] - image.shape[2]) // 2
    reshaped_image[x_offset:x_offset+image.shape[0], y_offset:y_offset+image.shape[1], z_offset:z_offset+image.shape[2]] = image
