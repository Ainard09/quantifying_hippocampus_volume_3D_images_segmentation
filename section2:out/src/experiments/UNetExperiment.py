...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: The 0 of x-xis represents the background, the y-xis represents the anterior and z-axis represents the posterior
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            sn = Sensitivity3d(pred_label, x["seg"])
            sp = Specificity3d(pred_label, x["seg"])
            dc_list.append(dc)
            jc_list.append(jc)
            sn_list.append(sn)
            sp_list.append(sp)
        ...
        print(f"Dice mean: {np.mean(dc_list)}")
        print(f"Sensitivity mean: {np.mean(sn_list)}")
        print(f"Specificity mean: {np.mean(sp_list)}")
        print(f"Jaccard mean: {np.mean(jc_list)}")
